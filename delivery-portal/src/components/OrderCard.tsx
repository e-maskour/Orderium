...
export default function OrderCard({ order }: OrderCardProps) {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: (status: 'to_delivery' | 'in_delivery' | 'delivered' | 'canceled') =>
      deliveryPerson
        ? deliveryService.updateOrderStatus(order.orderId, status, deliveryPerson.id)
  ...
  const getStatusLabel = () => {
    switch (order.status) {
      case 'to_delivery':
        return t('toDeliveryStatus');
      case 'in_delivery':
        return t('inDeliveryStatus');
      case 'delivered':
  ...
  return (
    <Card className="h-full">
      ...
      <div className="flex align-items-center justify-content-between mb-2">
        <span className="text-sm" style={{ color: 'var(--orderium-text-muted)' }}>
          #{order.orderNumber}
        </span>
        <Tag value={getStatusLabel()} severity={getStatusSeverity()} rounded />
      </div>
      <div className="text-xs mb-3" style={{ color: 'var(--orderium-text-muted)' }}>
      ...
      <div className="flex align-items-start gap-2 mb-2">
        <i
          className="pi pi-box text-white border-circle flex align-items-center justify-content-center"
          style={{
            width: '1.5rem',
            height: '1.5rem',
            fontSize: '0.7rem',
            background: 'var(--orderium-primary)',
          }}
        />
        <div className="flex-1 min-w-0">
          ...
          <a
            ...
            className="text-xs flex align-items-center gap-1 mt-1"
            style={{ color: 'var(--orderium-primary)' }}
          >
      ...
      <div className="flex align-items-start gap-2 mb-3">
        <i
          className="pi pi-map-marker border-circle flex align-items-center justify-content-center"
          style={{
            width: '1.5rem',
            height: '1.5rem',
            fontSize: '0.7rem',
            color: 'var(--orderium-primary)',
            background: 'rgba(223, 120, 23, 0.1)',
          }}
        />
        <span className="text-xs flex-1" style={{ color: 'var(--orderium-text-muted)' }}>
      ...
      {/* Order Summary */}
      <div className="text-xs font-medium mb-2" style={{ color: 'var(--orderium-text-muted)' }}>
        {t('orderSummary')}:
      </div>
      ...
      <div className="flex justify-content-between text-xs font-bold">
        ...
        <span style={{ color: 'var(--orderium-primary)' }}>
          {formatCurrency(order.totalAmount || 0, language)}
        </span>
      </div>
