...
export default function OrderDetail() {
  ...
  const resolvedOrder: Order | undefined =
    order ?? freshData?.orders.find((o: Order) => o.orderId === Number(orderId));
  ...
  const openGoogleMaps = () => {
    if (resolvedOrder.latitude && resolvedOrder.longitude) {
      window.open(
        `https://www.google.com/maps/dir/?api=1&destination=${resolvedOrder.latitude},${resolvedOrder.longitude}`,
        '_blank',
      );
    } else if (resolvedOrder.customerAddress) {
      window.open(
        `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(resolvedOrder.customerAddress)}`,
        '_blank',
      );
    }
  };
  ...
  const openWaze = () => {
    if (resolvedOrder.latitude && resolvedOrder.longitude) {
      window.open(
        `https://waze.com/ul?ll=${resolvedOrder.latitude},${resolvedOrder.longitude}&navigate=yes`,
        '_blank',
      );
    } else if (resolvedOrder.customerAddress) {
      window.open(
        `https://waze.com/ul?q=${encodeURIComponent(resolvedOrder.customerAddress)}&navigate=yes`,
        '_blank',
      );
    }
  };
      ...
      <div style={{ flex: 1, padding: '1rem 1rem 0' }}>
              ...
              {resolvedOrder.items.map((item, idx) => (
                <div
                  ...
                  style={{
                    ...
                    padding: '0.75rem 0',
                    borderBottom:
                      idx < resolvedOrder.items!.length - 1 ? '1px solid #f3f4f6' : 'none',
                  }}
                >
                  <div
                    style={{
        ...
        {resolvedOrder.status === 'delivered' && (
          ...
          >
            ...
            <p style={{ fontWeight: 800, color: '#15803d', fontSize: '1.1rem', margin: 0 }}>
              {t('orderDelivered')}
            </p>
          </div>
        )}
        {resolvedOrder.status === 'canceled' && (
          ...
          >
            <div style={{ fontSize: '2.5rem', marginBottom: '0.5rem' }}>❌</div>
            <p style={{ fontWeight: 800, color: '#dc2626', fontSize: '1.1rem', margin: 0 }}>
              {t('orderCanceled')}
            </p>
          </div>
        )}
