      ...
      <div className="bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl p-6 shadow-sm border border-slate-200">
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-center pb-3 border-b border-slate-200">
            <span className="text-sm font-medium text-slate-600">Sous-total HT</span>
            <span className="text-base font-semibold text-slate-900">{totalHT.toFixed(2)} DH</span>
          </div>
            ...
            <div className="space-y-2 py-2">
              {Object.entries(taxByRate).map(([rate, amount]) => (
                <div key={rate} className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-medium text-slate-700">{amount.toFixed(2)} DH</span>
                </div>
              ))}
            </div>
          )}
          ...
          {Object.keys(taxByRate).length > 0 && (
            <div className="flex justify-between items-center py-2 border-t border-slate-200">
              ...
              <span className="text-base font-semibold text-slate-800">{totalTVA.toFixed(2)} DH</span>
            </div>
          )}
          ...
          <div className="mt-4 pt-4 border-t-2 border-slate-300">
            <div className="bg-gradient-to-r from-amber-500 to-amber-600 rounded-lg p-4 shadow-md">
              <div className="flex justify-between items-center">
                <span className="text-white font-bold text-base">Total TTC</span>
                <span className="text-white font-bold text-2xl">{totalTTC.toFixed(2)} DH</span>
              </div>
            </div>
          </div>
        </div>
