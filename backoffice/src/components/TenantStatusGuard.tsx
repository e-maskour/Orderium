...
// ─── Modules context ──────────────────────────────────────────────────────────
// Provides the resolved modules config (or null while loading) to all consumers.
...
const DEFAULT_MODULES: TenantModules = {
  ...
  warehouse: true,
  category: true,
  portals: { clientPortal: false, deliveryPortal: false },
};
...
function detectBlockedReason(error: ApiError): BlockedReason | null {
  ...
  if (msg.includes('trial') && (msg.includes('ended') || msg.includes('expired')))
...
export function TenantStatusGuard({ children }: Props) {
  const [tenantInfo, setTenantInfo] = useState<TenantPublicStatus | null | undefined>(
    cachedTenantInfo,
  );
