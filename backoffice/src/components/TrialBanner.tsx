...
export function TrialBanner({ daysRemaining, trialEndsAt }: Props) {
    ...
    const endsText = trialEndsAt
        ? new Date(trialEndsAt).toLocaleDateString(undefined, { day: '2-digit', month: 'short', year: 'numeric' })
        : null;
    ...
    return (
        <div className={`relative flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-white ${bgClass}`}>
