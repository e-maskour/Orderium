...
describe('StockMovement model', () => {
  ...
  describe('isCancelled', () => {
    it('returns true for cancelled status', () => {
      expect(new StockMovement(makeMovementData({ status: 'cancelled' }) as any).isCancelled).toBe(
        true,
      );
    });
  });
  ...
  describe('canValidate', () => {
    it('returns true for draft/waiting/confirmed/assigned', () => {
      for (const status of ['draft', 'waiting', 'confirmed', 'assigned']) {
        expect(new StockMovement(makeMovementData({ status }) as any).canValidate).toBe(true);
      }
    });

    it('returns false for done and cancelled', () => {
      for (const status of ['done', 'cancelled']) {
        expect(new StockMovement(makeMovementData({ status }) as any).canValidate).toBe(false);
      }
    });
  });
  ...
  describe('canCancel', () => {
    ...
    it('returns false for cancelled status', () => {
      expect(new StockMovement(makeMovementData({ status: 'cancelled' }) as any).canCancel).toBe(
        false,
      );
    });
  });
  ...
  describe('isInbound', () => {
    it('returns true for receipt', () => {
      expect(
        new StockMovement(makeMovementData({ movementType: 'receipt' }) as any).isInbound,
      ).toBe(true);
    });

    it('returns true for return_in', () => {
      expect(
        new StockMovement(makeMovementData({ movementType: 'return_in' }) as any).isInbound,
      ).toBe(true);
    });

    it('returns true for production_in', () => {
      expect(
        new StockMovement(makeMovementData({ movementType: 'production_in' }) as any).isInbound,
      ).toBe(true);
    });

    it('returns false for outbound types', () => {
      for (const type of ['delivery', 'return_out', 'production_out', 'scrap']) {
        expect(new StockMovement(makeMovementData({ movementType: type }) as any).isInbound).toBe(
          false,
        );
      }
    });
  ...
  describe('isOutbound', () => {
    it('returns true for delivery', () => {
      expect(
        new StockMovement(makeMovementData({ movementType: 'delivery' }) as any).isOutbound,
      ).toBe(true);
    });

    it('returns true for scrap', () => {
      expect(new StockMovement(makeMovementData({ movementType: 'scrap' }) as any).isOutbound).toBe(
        true,
      );
    });
    ...
    it('returns false for receipt', () => {
      expect(
        new StockMovement(makeMovementData({ movementType: 'receipt' }) as any).isOutbound,
      ).toBe(false);
    });

    it('returns false for internal', () => {
      expect(
        new StockMovement(makeMovementData({ movementType: 'internal' }) as any).isOutbound,
      ).toBe(false);
    });
  });
  ...
  describe('isInternal', () => {
    it('returns true only for internal type', () => {
      expect(
        new StockMovement(makeMovementData({ movementType: 'internal' }) as any).isInternal,
      ).toBe(true);
      expect(
        new StockMovement(makeMovementData({ movementType: 'receipt' }) as any).isInternal,
      ).toBe(false);
    });
  });
  ...
  describe('productDisplayName', () => {
    ...
    it('falls back to Product #id when no product relation', () => {
      const m = new StockMovement(makeMovementData({ product: undefined, productId: 42 }) as any);
...
describe('StockQuant model', () => {
  ...
  describe('utilizationRate', () => {
    it('calculates percentage of qty that is reserved', () => {
      const quant = StockQuant.fromApiResponse(
        makeQuantData({ quantity: 100, reservedQuantity: 25 }),
      );
    ...
    it('returns 100 when all stock is reserved', () => {
      const quant = StockQuant.fromApiResponse(
        makeQuantData({ quantity: 50, reservedQuantity: 50 }),
      );
  ...
  describe('displayQuantity', () => {
    it('returns "available / total" format', () => {
      const quant = StockQuant.fromApiResponse(
        makeQuantData({ availableQuantity: 90, quantity: 100 }),
      );
