...
export class StockMovement implements IStockMovement {
  ...
  static fromApiResponse(data: any): StockMovement {
    return new StockMovement({
      ...
      sourceWarehouseName: data.sourceWarehouseName ?? data.sourceWarehouse?.name,
      destWarehouseId: data.destWarehouseId,
      destWarehouseName: data.destWarehouseName ?? data.destWarehouse?.name,
      quantity: parseFloat(data.quantity) || 0,
      unitOfMeasureId: data.unitOfMeasureId,
      unitOfMeasureCode: data.unitOfMeasureCode ?? data.unitOfMeasure?.code,
      status: data.status ?? 'draft',
