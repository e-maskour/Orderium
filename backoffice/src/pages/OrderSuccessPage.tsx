...
import { CheckCircle, Home, Download, Printer, Eye, FileText, Receipt as ReceiptIcon, Share2 } from 'lucide-react';
...
export default function OrderSuccessPage() {
  ...
  const documents: { type: 'receipt' | 'delivery-note'; label: string; subtitle: string; icon: React.ReactNode; accentColor: string }[] = [
    {
      type: 'receipt',
      label: t('receipt'),
  ...
  return (
    <>
      <div style={{ minHeight: '100vh', background: '#f3f4f6', display: 'flex', flexDirection: 'column' }}>
        ...
        {/* Success Hero */}
        <div style={{ background: '#ffffff', borderBottom: '1px solid #e5e7eb', padding: '2rem 1.5rem' }}>
        ...
        {/* Content */}
        <div style={{ flex: 1, maxWidth: '40rem', margin: '0 auto', width: '100%', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>

          {/* Summary strip */}
          <div style={{ background: '#ffffff', borderRadius: '0.75rem', border: '1px solid #e5e7eb', overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)' }}>
              <div style={{ padding: '1rem', borderRight: '1px solid #f3f4f6', textAlign: 'center' }}>
                <p style={{ fontSize: '0.6875rem', fontWeight: 500, color: '#9ca3af', margin: '0 0 0.25rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{t('total')}</p>
                <p style={{ fontSize: '1rem', fontWeight: 700, color: '#111827', margin: 0 }}>{formatCurrency(state.total, language as 'fr' | 'ar')}</p>
              </div>
              <div style={{ padding: '1rem', borderRight: '1px solid #f3f4f6', textAlign: 'center', background: '#f0fdf4' }}>
                <p style={{ fontSize: '0.6875rem', fontWeight: 500, color: '#059669', margin: '0 0 0.25rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Paid</p>
                <p style={{ fontSize: '1rem', fontWeight: 700, color: '#059669', margin: 0 }}>{formatCurrency(state.paidAmount, language as 'fr' | 'ar')}</p>
              </div>
              <div style={{ padding: '1rem', textAlign: 'center' }}>
                <p style={{ fontSize: '0.6875rem', fontWeight: 500, color: '#9ca3af', margin: '0 0 0.25rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{t('change')}</p>
                <p style={{ fontSize: '1rem', fontWeight: 700, color: '#1d4ed8', margin: 0 }}>{formatCurrency(state.change, language as 'fr' | 'ar')}</p>
              </div>
            </div>
          </div>
            ...
            {documents.map((doc, idx) => (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontWeight: 600, fontSize: '0.875rem', color: '#111827', margin: 0 }}>{doc.label}</p>
                ...
                <div style={{ display: 'flex', gap: '0.375rem', marginLeft: '0.75rem' }}>
                  ...
                  <Button
                    ...
                    tooltip="Download"
                    tooltipOptions={{ position: 'top' }}
                    style={{ width: '2.25rem', height: '2.25rem', background: doc.accentColor, border: 'none', color: '#ffffff' }}
                  />
                </div>
              </div>
            ))}
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            <Button
              ...
              style={{ flex: 1, height: '2.75rem', background: '#10b981', border: 'none', fontWeight: 600 }}
            />
            <Button
              onClick={handleShare}
              outlined
              ...
              label="Share"
              style={{ height: '2.75rem', paddingInline: '1.25rem', fontWeight: 600, borderColor: '#d1d5db', color: '#374151' }}
            />
          </div>
