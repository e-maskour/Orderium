...
export default function PaiementsAchat() {
  ...
  const { data: filteredPayments = [], isLoading: loading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return all.filter((p: Payment) => p.supplierId != null);
    },
  });

  const { data: allPurchasePayments = [] } = useQuery({
    queryKey: ['payments-achat-kpi'],
    queryFn: async () =>
      (await paymentsService.getAll({})).filter((p: Payment) => p.supplierId != null),
    staleTime: 60_000,
  });
  ...
  const getSupplierName = (invoiceId: number) => {
    const inv = invoices.find((i: any) => i.invoice.id === invoiceId);
    return inv?.invoice.supplierName || '-';
  };
  ...
  const totalAmount = allPurchasePayments.reduce((sum: number, p: Payment) => sum + p.amount, 0);
  const thisMonthPayments = allPurchasePayments.filter((p: Payment) => {
    const d = new Date(p.paymentDate);
    const n = new Date();
    return d.getMonth() === n.getMonth() && d.getFullYear() === n.getFullYear();
  });
  const thisMonthAmount = thisMonthPayments.reduce((sum: number, p: Payment) => sum + p.amount, 0);
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          <KpiGrid count={3}>
            <KpiCard
              ...
              value={allPurchasePayments.length}
              icon={CreditCard}
              color="blue"
            />
            <KpiCard
          ...
          <DataTable
            ...
            emptyMessage={
              <EmptyState
                ...
                description="Aucun paiement d'achat ne correspond à votre recherche"
              />
            }
