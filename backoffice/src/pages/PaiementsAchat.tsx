...
export default function PaiementsAchat() {
  ...
  const { data: allPayments = [], isLoading: loading } = useQuery({
    queryKey: ['payments-achat', debouncedSearch],
  ...
  // Only show purchase payments (supplierId set)
  const filteredPayments = allPayments.filter((p: Payment) => p.supplierId != null);
  ...
  const thisMonthPayments = filteredPayments.filter((p: Payment) => {
            ...
            <MobileList
              ...
              config={{
                ...
                bottomLeft: (p: Payment) =>
                  `${getSupplierName(p.invoiceId)} · ${new Date(p.paymentDate).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' })}`,
