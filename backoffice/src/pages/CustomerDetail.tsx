...
export default function CustomerDetail() {
  ...
  const { data: analytics, isLoading: analyticsLoading } = useQuery({
    queryKey: ['customer-analytics', id, selectedYear],
    queryFn: () => partnersService.getCustomerAnalytics(Number(id), selectedYear),
    enabled: !!id,
  });
  ...
  const { totalInvoices, totalRevenue, paidAmount, unpaidAmount, averagePerInvoice } = kpis;
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: '0.875rem', color: '#64748b' }}>{t('customerDetails')}</p>
                </div>
              </div>
            </div>
            <Button
            ...
            <div style={{ background: 'linear-gradient(to bottom right, white, #f8fafc)', borderRadius: '1rem', padding: '2rem', border: '1px solid rgba(226,232,240,0.6)', boxShadow: '0 10px 15px -3px rgba(226,232,240,0.4)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <div style={{ padding: '0.625rem', background: 'linear-gradient(to bottom right, #3b82f6, #2563eb)', borderRadius: '0.75rem', boxShadow: '0 10px 15px -3px rgba(59,130,246,0.3)' }}>
              ...
              <div style={{ position: 'relative' }}>
                {chartData.every(d => d.count === 0 && d.amount === 0) ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                    <div style={{ position: 'relative', borderRadius: '1rem', padding: '2rem', border: '1px solid rgba(226,232,240,0.6)', boxShadow: '0 20px 25px -5px rgba(219,234,254,0.5)', overflow: 'visible', zIndex: 10 }}>
                          ...
                          options={{
                            ...
                            fill: {
                              ...
                              gradient: {
                                ...
                                gradientToColors: ['#60A5FA'], inverseColors: false,
                                opacityFrom: 0.6, opacityTo: 0.05, stops: [0, 90, 100]
                              }
                            },
                            colors: ['#3B82F6'],
                            markers: {
                              size: 5, colors: ['#3B82F6'], strokeColors: '#fff', strokeWidth: 3,
                              hover: { size: 8, sizeOffset: 3 }, shape: 'circle',
                              discrete: chartData.map((data, index) => ({
                                seriesIndex: 0, dataPointIndex: index,
                                fillColor: data.count > 0 ? '#3B82F6' : '#94A3B8',
                                strokeColor: '#fff', size: data.count > 0 ? 6 : 4,
                              }))
                            },
                            grid: {
                            ...
                            xaxis: {
                              ...
                              labels: {
                                ...
                                offsetY: 5, rotate: -45, rotateAlways: false,
                                hideOverlappingLabels: true, trim: true
                              },
                              axisBorder: { show: false }, axisTicks: { show: false },
                              crosshairs: {
                                show: true, width: 1,
                                stroke: { color: '#3B82F6', width: 2, dashArray: 0 },
                                dropShadow: { enabled: true, top: 0, left: 0, blur: 4, opacity: 0.4 }
                              }
                            },
                            yaxis: {
                              labels: {
                            ...
                            tooltip: {
                              ...
                              theme: 'dark', style: { fontSize: '13px', fontFamily: 'inherit' },
                              x: {
                                show: true,
                                formatter: (value, { dataPointIndex }) => `${chartData[dataPointIndex].month} ${selectedYear}`
                              },
                              y: {
                            ...
                            responsive: [
                              {
                                breakpoint: 640,
                                options: {
                                  chart: { height: 280 }, stroke: { width: 2 }, markers: { size: 3 },
                                  grid: { padding: { top: 0, right: 5, bottom: 0, left: -5 } },
                                  xaxis: { labels: { style: { fontSize: '10px' }, rotate: -45, rotateAlways: true } },
                                  yaxis: { labels: { style: { fontSize: '9px' }, offsetX: -2 }, title: { text: undefined } },
                                  tooltip: { style: { fontSize: '11px' } }
                                }
                              },
                              {
                                breakpoint: 1024,
                                options: { chart: { height: 350 }, xaxis: { labels: { rotate: 0 } } }
                              }
                            ]
                          } as ApexOptions}
                          series={[{ name: 'Revenus', data: chartData.map(d => d.amount) }]}
                          type="area"
