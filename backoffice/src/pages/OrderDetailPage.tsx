...
import {
    ArrowLeft, Package, Phone, MapPin, Printer, Pencil, Trash2,
    Plus, Check, X, CreditCard, ChevronRight, Clock, User,
    Receipt, Truck, Calendar, ShoppingBag
} from 'lucide-react';

// ─── Status config ──────────────────────────────────────────────────────────

const STATUS_CFG: Record<string, { label_fr: string; label_ar: string; bg: string; color: string; border: string; icon: string }> = {
    pending: { label_fr: 'En attente', label_ar: 'في الانتظار', bg: '#f8fafc', color: '#475569', border: '#e2e8f0', icon: '⏳' },
    confirmed: { label_fr: 'Confirmé', label_ar: 'مؤكد', bg: '#eff6ff', color: '#1d4ed8', border: '#bfdbfe', icon: '✅' },
    in_progress: { label_fr: 'En cours', label_ar: 'قيد التنفيذ', bg: '#fffbeb', color: '#92400e', border: '#fde68a', icon: '⚙️' },
    picked_up: { label_fr: 'Récupéré', label_ar: 'تم الاستلام', bg: '#f5f3ff', color: '#6d28d9', border: '#ddd6fe', icon: '📦' },
    to_delivery: { label_fr: 'À livrer', label_ar: 'للتوصيل', bg: '#fffbeb', color: '#b45309', border: '#fde68a', icon: '🚚' },
    in_delivery: { label_fr: 'En livraison', label_ar: 'في التوصيل', bg: '#ecfeff', color: '#0e7490', border: '#a5f3fc', icon: '🚴' },
    delivered: { label_fr: 'Livré', label_ar: 'تم التوصيل', bg: '#ecfdf5', color: '#047857', border: '#a7f3d0', icon: '✔️' },
    cancelled: { label_fr: 'Annulé', label_ar: 'ملغى', bg: '#fef2f2', color: '#b91c1c', border: '#fecaca', icon: '❌' },
    canceled: { label_fr: 'Annulé', label_ar: 'ملغى', bg: '#fef2f2', color: '#b91c1c', border: '#fecaca', icon: '❌' },
};
...
export default function OrderDetailPage() {
    ...
    const computedTotal = editMode ? Math.max(0, computedSubtotal - globalDiscount) : (order?.total ?? 0);
    ...
    const handleSave = () => {
        ...
        updateItemsMutation.mutate({ items: mappedItems, subtotal, total, discount: globalDiscount, discountType: order?.discountType ?? 0, tax: 0 });
    };
    ...
    const createPaymentMutation = useMutation({
        mutationFn: () => orderPaymentsService.create({
            orderId: Number(id),
        ...
        return (
            <AdminLayout>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '20rem' }}>
                    <div style={{ width: '2rem', height: '2rem', borderRadius: '9999px', borderBottom: '2px solid #235ae4' }} className="animate-spin" />
                </div>
            </AdminLayout>
        );
    }
        ...
        return (
            <AdminLayout>
                <div style={{ textAlign: 'center', padding: '3rem', color: '#64748b' }}>
                    <Package style={{ width: '3rem', height: '3rem', margin: '0 auto 1rem', color: '#cbd5e1' }} />
                    <p>{t('orderNotFound') || 'Commande introuvable'}</p>
                    <Button label={t('back') || 'Retour'} onClick={() => navigate('/orders')} style={{ marginTop: '1rem' }} />
                </div>
            </AdminLayout>
        );
    }
    ...
    const formattedDate = orderDate.toLocaleDateString(isAr ? 'ar-MA' : 'fr-FR', { day: 'numeric', month: 'long', year: 'numeric' });
    ...
    const paymentTypeOptions = Object.entries(ORDER_PAYMENT_TYPE_LABELS).map(([value, label]) => ({ value, label }));
            ...
            <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 0 3rem' }}>
                ...
                <div className="doc-detail-hdr">
                    ...
                    <div className="doc-detail-hdr__body">
                        <div className="doc-detail-hdr__crumb">
                            ...
                            <span style={{ fontSize: '0.6875rem', fontWeight: 700, color: '#235ae4', textTransform: 'uppercase', letterSpacing: '0.07em' }}>#{order.orderNumber}</span>
                        </div>
                    ...
                    <div className="doc-detail-hdr__badges">
                        {/* Status badge */}
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4375rem', padding: '0.4375rem 0.875rem', borderRadius: '9999px', background: statusCfg.bg, border: `1.5px solid ${statusCfg.border}`, whiteSpace: 'nowrap', flexShrink: 0 }}>
                            <span style={{ fontSize: '0.875rem', lineHeight: 1 }}>{statusCfg.icon}</span>
                            <span style={{ fontSize: '0.6875rem', fontWeight: 700, color: statusCfg.color, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{statusLabel}</span>
                        </div>
                        {/* Date badge */}
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4375rem', padding: '0.4375rem 0.875rem', borderRadius: '9999px', background: '#f8fafc', border: '1.5px solid #e2e8f0', whiteSpace: 'nowrap', flexShrink: 0 }}>
                            <Calendar style={{ width: '0.75rem', height: '0.75rem', color: '#94a3b8', flexShrink: 0 }} />
                            <span style={{ fontSize: '0.6875rem', fontWeight: 600, color: '#64748b', letterSpacing: '0.02em' }}>{formattedDate}</span>
                        </div>
                        {/* Print buttons */}
                        <Button
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
                        {nextActions.map(action => (
                            <button
                                key={action.value}
                                type="button"
                                onClick={() => changeStatusMutation.mutate(action.value)}
                                disabled={changeStatusMutation.isPending}
                                style={{
                                    display: 'inline-flex', alignItems: 'center', gap: '0.375rem',
                                    padding: '0.4rem 1rem', borderRadius: '9999px', fontSize: '0.8125rem', fontWeight: 600,
                                    cursor: 'pointer', border: `1.5px solid ${action.border}`,
                                    backgroundColor: action.bg, color: action.color, transition: 'opacity 0.15s',
                                }}
                            >
                ...
                <div className="ord-detail-grid">
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                        ...
                        {/* Note */}
                        {order.notes && (
                            <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: '0.75rem', padding: '0.75rem 1rem', fontSize: '0.875rem', color: '#92400e', display: 'flex', gap: '0.5rem', alignItems: 'flex-start' }}>
                        ...
                        <div style={{ background: '#fff', borderRadius: '0.875rem', border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                            ...
                            <div className="ord-items-card-hdr">
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                                    ...
                                    <span style={{ fontSize: '0.75rem', color: '#64748b', background: '#e2e8f0', borderRadius: '9999px', padding: '0.1rem 0.5rem', fontWeight: 600 }}>
                                ...
                                <div className="ord-items-card-hdr-actions">
                                    {editMode ? (
                                        <>
                                            <button
                                                type="button"
                                                onClick={() => setNewItem({ description: '', quantity: '1', unitPrice: '0' })}
                                                style={{ display: 'inline-flex', alignItems: 'center', gap: '0.25rem', padding: '0.3rem 0.75rem', borderRadius: '9999px', background: '#eff6ff', border: '1.5px solid #bfdbfe', color: '#1d4ed8', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer' }}
                                            >
                                            ...
                                            <button
                                                ...
                                                style={{ display: 'inline-flex', alignItems: 'center', gap: '0.25rem', padding: '0.3rem 0.75rem', borderRadius: '9999px', background: '#dcfce7', border: '1.5px solid #86efac', color: '#16a34a', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer' }}
                                            >
                                            ...
                                            <button
                                                ...
                                                style={{ display: 'inline-flex', alignItems: 'center', gap: '0.25rem', padding: '0.3rem 0.75rem', borderRadius: '9999px', background: '#f1f5f9', border: '1.5px solid #e2e8f0', color: '#64748b', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer' }}
                                            >
                                        ...
                                        isEditable && (
                                            <button
                                                ...
                                                style={{ display: 'inline-flex', alignItems: 'center', gap: '0.25rem', padding: '0.3rem 0.75rem', borderRadius: '9999px', background: '#f8fafc', border: '1.5px solid #e2e8f0', color: '#64748b', fontSize: '0.75rem', fontWeight: 600, cursor: 'pointer' }}
                                            >
                            ...
                            <div className="ord-items-thead">
                                {[isAr ? 'المقال' : 'Article', isAr ? 'الكمية' : 'Qté', isAr ? 'السعر' : 'Prix unit.', isAr ? 'الإجمالي' : 'Total'].map((h) => (
                                    <span key={h} style={{ fontSize: '0.7rem', fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{h}</span>
                                ))}
                            </div>
                            ...
                            {computedItems.length === 0 && !newItem ? (
                                ...
                                <div>
                                    {computedItems.map((item: any, idx: number) => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                style={{
                                                    borderBottom: idx < computedItems.length - 1 ? '1px solid #e2e8f0' : 'none',
                                                ...
                                                {editMode ? (
                                                    <>
                                                        ...
                                                        <div style={{ paddingInlineEnd: '0.5rem' }}>
                                                            <p style={{ margin: 0, fontSize: '0.875rem', fontWeight: 600, color: '#0f172a' }}>{name}</p>
                                                        </div>
                                                        {/* Qty */}
                                                        <input
                                                            type="number" min="0" step="0.01" value={item.quantity}
                                                            onChange={e => setEditItems(prev => prev.map((it, i) => i === idx ? { ...it, quantity: e.target.value } : it))}
                                                            className="ord-item-qty-input"
                                                            style={{ width: '4rem', height: '2rem', padding: '0 0.4rem', borderRadius: '0.375rem', border: '1.5px solid #fde68a', fontWeight: 700, color: '#b45309', background: '#fef3c7', textAlign: 'center', fontSize: '0.8125rem' }}
                                                        />
                                                        {/* Unit price */}
                                                        <input
                                                            type="number" min="0" step="0.01" value={item.unitPrice}
                                                            onChange={e => setEditItems(prev => prev.map((it, i) => i === idx ? { ...it, unitPrice: e.target.value } : it))}
                                                            className="ord-item-price-input"
                                                            style={{ width: '5rem', height: '2rem', padding: '0 0.4rem', borderRadius: '0.375rem', border: '1.5px solid #e2e8f0', textAlign: 'right', fontSize: '0.8125rem' }}
                                                        />
                                                        {/* Delete */}
                                                        <button
                                                            type="button"
                                                            ...
                                                            style={{ width: '2rem', height: '2rem', borderRadius: '0.375rem', background: '#fef2f2', border: '1px solid #fecaca', color: '#dc2626', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
                                                        >
                                                    ...
                                                    <>
                                                        ...
                                                        <div style={{ minWidth: 0 }}>
                                                            <p className="ord-item-name" style={{ margin: 0, fontSize: '0.875rem', fontWeight: 600, color: '#0f172a' }}>{name}</p>
                                                            {disc > 0 && <span style={{ fontSize: '0.7rem', color: '#dc2626' }}>- {formatAmount(disc, 2)}</span>}
                                                        </div>
                                                        {/* Qty badge */}
                                                        <div style={{ width: '2rem', height: '2rem', background: '#fef3c7', borderRadius: '0.375rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                            <span style={{ fontSize: '0.75rem', fontWeight: 800, color: '#b45309' }}>{Number.isInteger(qty) ? qty : qty.toFixed(1)}</span>
                                                        </div>
                                                        {/* Unit price */}
                                                        <span style={{ fontSize: '0.8125rem', color: '#475569' }}>{formatAmount(unitPrice, 2)}</span>
                                                        {/* Total */}
                                                        <span className="ord-item-total" style={{ fontSize: '0.875rem', fontWeight: 700, color: '#0f172a' }}>
                                                            {formatAmount(lineTotal, 2)} <span style={{ fontSize: '0.7rem', color: '#94a3b8', fontWeight: 400 }}>{t('currency')}</span>
                                                        </span>
                                                    </>
                                                )}
                                            </div>
                                    ...
                                    {editMode && newItem !== null && (
                                        <div className="ord-new-item-row" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.625rem 1rem', background: '#f0fdf4', borderTop: '1px solid #bbf7d0' }}>
                                            <div style={{ flex: 1, position: 'relative' }}>
                                                ...
                                                {showSuggestions && productSuggestions.length > 0 && (
                                                    <div style={{ position: 'absolute', bottom: '2.25rem', left: 0, right: 0, background: '#fff', border: '1px solid #e2e8f0', borderRadius: '0.5rem', boxShadow: '0 -4px 16px rgba(0,0,0,0.12)', zIndex: 50, maxHeight: '12rem', overflowY: 'auto' }}>
                                            ...
                                            <div className="ord-new-item-inputs">
                                                <input type="number" min="0" placeholder="Qté" value={newItem.quantity} onChange={e => setNewItem(prev => prev ? { ...prev, quantity: e.target.value } : null)} style={{ width: '3.5rem', height: '2rem', padding: '0 0.4rem', borderRadius: '0.375rem', border: '1.5px solid #86efac', fontSize: '0.8125rem', textAlign: 'center' }} />
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        if (newItem.description && parseFloat(newItem.quantity) > 0) {
                                                            ...
                                                            setNewItem(null); setProductSuggestions([]);
                                                        }
                                                    }}
                                                    style={{ width: '2rem', height: '2rem', borderRadius: '0.375rem', background: '#dcfce7', border: '1px solid #86efac', color: '#16a34a', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
                                                >
                                                    <Check style={{ width: '0.875rem', height: '0.875rem' }} />
                                                </button>
                                                <button
                                                    type="button"
                                                    onClick={() => { setNewItem(null); setProductSuggestions([]); setShowSuggestions(false); }}
                                                    style={{ width: '2rem', height: '2rem', borderRadius: '0.375rem', background: '#f1f5f9', border: '1px solid #e2e8f0', color: '#64748b', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
                                                >
                            ...
                            {/* Totals footer */}
                            <div style={{ padding: '0.875rem 1rem', borderTop: '2px solid #f1f5f9', background: '#f8fafc', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                {globalDiscount > 0 && (
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'linear-gradient(135deg,#1d4ed8,#235ae4)', borderRadius: '0.625rem', padding: '0.625rem 0.875rem' }}>
                                    <span style={{ fontSize: '0.9375rem', fontWeight: 700, color: '#fff' }}>{t('grandTotal')}</span>
                                    <span style={{ fontSize: '1.25rem', fontWeight: 800, color: '#fff' }}>
                                        {formatAmount(computedTotal, 2)} <span style={{ fontSize: '0.875rem', fontWeight: 500 }}>{t('currency')}</span>
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        {(paidAmount > 0 || remainingAmount > 0) && !editMode && (
                            <div style={{ background: '#fff', borderRadius: '0.875rem', border: '1px solid #e2e8f0', padding: '1rem', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                                <div className="ord-pay-progress-labels" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                                    <span style={{ fontSize: '0.8125rem', fontWeight: 600, color: '#047857' }}>
                                        {t('paidAmount')}: {formatAmount(paidAmount, 2)} {t('currency')}
                                    </span>
                                    <span style={{ fontSize: '0.8125rem', fontWeight: 600, color: remainingAmount > 0 ? '#dc2626' : '#047857' }}>
                                        {t('remainingAmount')}: {formatAmount(remainingAmount, 2)} {t('currency')}
                                    </span>
                                </div>
                                <div style={{ height: '0.5rem', borderRadius: '9999px', background: '#fee2e2', overflow: 'hidden' }}>
                                    <div style={{ height: '100%', borderRadius: '9999px', background: 'linear-gradient(to right, #10b981, #047857)', width: `${Math.min(100, computedTotal > 0 ? (paidAmount / computedTotal) * 100 : 0)}%`, transition: 'width 0.4s', }} />
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                        ...
                        {/* Customer card */}
                        <div style={{ background: '#fff', borderRadius: '0.875rem', border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                            <div style={{ padding: '0.75rem 1rem', background: 'linear-gradient(135deg, #1d4ed8, #235ae4)' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                    <User style={{ width: '0.875rem', height: '0.875rem', color: 'rgba(255,255,255,0.8)' }} />
                            ...
                            <div style={{ padding: '1rem' }}>
                                {order.customerName && (
                                    <p style={{ margin: '0 0 0.5rem', fontSize: '1rem', fontWeight: 700, color: '#0f172a' }}>{order.customerName}</p>
                                )}
                                {order.customerPhone && (
                                    <a href={`tel:${order.customerPhone}`} style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', fontSize: '0.875rem', color: '#235ae4', textDecoration: 'none', marginBottom: '0.375rem' }}>
                                        <Phone style={{ width: '0.875rem', height: '0.875rem', flexShrink: 0 }} />{order.customerPhone}
                                    </a>
                                )}
                                {order.customerAddress && (
                                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.375rem', fontSize: '0.8125rem', color: '#475569' }}>
                                        <MapPin style={{ width: '0.875rem', height: '0.875rem', flexShrink: 0, marginTop: '0.1rem' }} />{order.customerAddress}
                                    </div>
                                )}
                        ...
                        <div style={{ background: '#fff', borderRadius: '0.875rem', border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                            ...
                            <div style={{ padding: '0.875rem 1rem', display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.625rem' }}>
                                    <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '0.625rem', padding: '0.625rem' }}>
                                        <p style={{ margin: '0 0 0.125rem', fontSize: '0.7rem', fontWeight: 600, color: '#047857', textTransform: 'uppercase' }}>{t('paidAmount')}</p>
                                        <p style={{ margin: 0, fontSize: '1rem', fontWeight: 800, color: '#047857' }}>{formatAmount(paidAmount, 2)} <span style={{ fontSize: '0.75rem', fontWeight: 400 }}>{t('currency')}</span></p>
                                    </div>
                                    <div style={{ background: remainingAmount > 0 ? '#fef2f2' : '#f0fdf4', border: `1px solid ${remainingAmount > 0 ? '#fecaca' : '#bbf7d0'}`, borderRadius: '0.625rem', padding: '0.625rem' }}>
                                        <p style={{ margin: '0 0 0.125rem', fontSize: '0.7rem', fontWeight: 600, color: remainingAmount > 0 ? '#dc2626' : '#047857', textTransform: 'uppercase' }}>{t('remainingAmount')}</p>
                                        <p style={{ margin: 0, fontSize: '1rem', fontWeight: 800, color: remainingAmount > 0 ? '#dc2626' : '#047857' }}>{formatAmount(remainingAmount, 2)} <span style={{ fontSize: '0.75rem', fontWeight: 400 }}>{t('currency')}</span></p>
                                    </div>
                                </div>
                                ...
                                {payments.length > 0 && (
                                    <div>
                                        <p style={{ margin: '0 0 0.5rem', fontSize: '0.75rem', fontWeight: 600, color: '#64748b' }}>{t('paymentHistory')}</p>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem', maxHeight: '10rem', overflowY: 'auto' }}>
                                            {payments.map((p: any) => (
                                                <div key={p.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.5rem 0.625rem', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '0.5rem' }}>
                                                    <div>
                                                        <span style={{ fontSize: '0.875rem', fontWeight: 700, color: '#047857' }}>{formatAmount(p.amount, 2)} {t('currency')}</span>
                                                        <span style={{ fontSize: '0.7rem', color: '#64748b', marginInlineStart: '0.5rem' }}>{p.paymentDate} · {p.paymentType}</span>
                                                    </div>
                                                    <button
                                                        type="button"
                                                        onClick={() => toastConfirm(t('deletePayment') || 'Supprimer ?', () => deletePaymentMutation.mutate(p.id), { confirmLabel: t('delete') })}
                                                        style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#dc2626', padding: '0.125rem', display: 'flex' }}
                                                    >
                                                        <Trash2 style={{ width: '0.875rem', height: '0.875rem' }} />
                                                    </button>
                                                </div>
                                ...
                                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '0.875rem', display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                                    <p style={{ margin: 0, fontSize: '0.8125rem', fontWeight: 700, color: '#334155' }}>{t('addPayment')}</p>
                                    <div>
                                        <label style={{ fontSize: '0.7rem', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '0.25rem' }}>{t('amount')}</label>
                                        <InputNumber
                                            value={paymentAmount}
                                    ...
                                    <div>
                                        <label style={{ fontSize: '0.7rem', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '0.25rem' }}>{t('paymentDate')}</label>
                                        <InputText
                                    ...
                                    <div>
                                        <label style={{ fontSize: '0.7rem', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '0.25rem' }}>{t('paymentType')}</label>
                                        <Dropdown
                                            value={paymentType} onChange={e => setPaymentType(e.value)}
                                            options={paymentTypeOptions} optionLabel="label" optionValue="value"
                                            style={{ width: '100%' }}
                                        />
                                    </div>
                                    <div>
                                        <label style={{ fontSize: '0.7rem', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '0.25rem' }}>{t('notes')}</label>
                                        <InputText
                                            value={paymentNote} onChange={e => setPaymentNote(e.target.value)}
                                            placeholder={t('optional')} style={{ width: '100%' }}
                                        />
                                    </div>
                                    <Button
                                        label={t('addPayment')}
                                        icon={<CreditCard style={{ width: '0.875rem', height: '0.875rem', marginInlineEnd: '0.375rem' }} />}
                        ...
                        {/* Print actions */}
                        <div style={{ background: '#fff', borderRadius: '0.875rem', border: '1px solid #e2e8f0', padding: '0.875rem 1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                            <p style={{ margin: '0 0 0.25rem', fontSize: '0.75rem', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                                <Printer style={{ width: '0.8rem', height: '0.8rem', display: 'inline', marginInlineEnd: '0.375rem' }} />
                                {isAr ? 'طباعة المستندات' : 'Documents'}
                            </p>
                            <button
                                type="button"
                                onClick={() => handlePrint('receipt')}
                                style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', padding: '0.625rem 0.75rem', borderRadius: '0.5rem', background: '#f8fafc', border: '1px solid #e2e8f0', cursor: 'pointer', width: '100%', textAlign: 'left', fontFamily: 'inherit' }}
                            >
                                <Receipt style={{ width: '1.125rem', height: '1.125rem', color: '#235ae4', flexShrink: 0 }} />
                                <span style={{ fontSize: '0.875rem', fontWeight: 600, color: '#1e293b' }}>{isAr ? 'طباعة الوصل' : 'Imprimer le reçu'}</span>
                            </button>
                            <button
                                type="button"
                                onClick={() => handlePrint('delivery-note')}
                                style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', padding: '0.625rem 0.75rem', borderRadius: '0.5rem', background: '#f8fafc', border: '1px solid #e2e8f0', cursor: 'pointer', width: '100%', textAlign: 'left', fontFamily: 'inherit' }}
                            >
                                <Truck style={{ width: '1.125rem', height: '1.125rem', color: '#235ae4', flexShrink: 0 }} />
                                <span style={{ fontSize: '0.875rem', fontWeight: 600, color: '#1e293b' }}>{isAr ? 'بون التسليم' : 'Bon de livraison'}</span>
                            </button>
                        </div>

                        {/* Meta info */}
                        <div style={{ background: '#f8fafc', borderRadius: '0.875rem', border: '1px solid #e2e8f0', padding: '0.875rem 1rem' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8125rem' }}>
                                    <span style={{ color: '#64748b' }}>{isAr ? 'تاريخ الإنشاء' : 'Créée le'}</span>
                                    <span style={{ fontWeight: 600, color: '#334155' }}>{new Date(order.dateCreated).toLocaleDateString(isAr ? 'ar-MA' : 'fr-FR')}</span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8125rem' }}>
                                    <span style={{ color: '#64748b' }}>{isAr ? 'آخر تحديث' : 'Modifiée le'}</span>
                                    <span style={{ fontWeight: 600, color: '#334155' }}>{new Date(order.dateUpdated).toLocaleDateString(isAr ? 'ar-MA' : 'fr-FR')}</span>
                                </div>
