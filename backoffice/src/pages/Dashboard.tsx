...
import {
    ...
    Truck, XCircle, Package, RefreshCw, Star,
} from 'lucide-react';
...
// ─── Big KPI Card ──────────────────────────────────────────────────────────
function BigCard({
    icon: Icon, label, value, bg, iconBg, iconColor, onClick, compact,
}: {
    ...
    return (
        ...
        >
            ...
            <div style={{ fontSize: compact ? '0.65rem' : '0.75rem', fontWeight: 600, color: '#64748b', lineHeight: 1.2 }}>{label}</div>
            <div style={{ fontSize: compact ? '1rem' : '1.375rem', fontWeight: 800, color: '#0f172a', lineHeight: 1.1, letterSpacing: '-0.02em' }}>
                {value}
            </div>
        </div>
    );
...
// ─── Status Pill ───────────────────────────────────────────────────────────
function StatusPill({ icon: Icon, label, count, bg, color, iconBg, onClick }: {
    icon: React.ElementType; label: string; count: number;
    bg: string; color: string; iconBg: string; onClick?: () => void;
}) {
    return (
        <div
            onClick={onClick}
            style={{
                background: bg, borderRadius: '1rem', padding: '1rem',
                display: 'flex', flexDirection: 'column', alignItems: 'center',
                gap: '0.5rem', cursor: onClick ? 'pointer' : 'default',
                border: `2px solid ${color}22`,
                transition: 'transform 0.15s ease',
            }}
            onMouseEnter={e => { if (onClick) (e.currentTarget as HTMLElement).style.transform = 'scale(1.03)'; }}
        ...
        >
            ...
            <span style={{ fontSize: '0.75rem', fontWeight: 600, color, textAlign: 'center', lineHeight: 1.3 }}>{label}</span>
        </div>
    );
}
...
export default function Dashboard() {
    ...
    const { data: recentOrdersData } = useQuery({
        ...
        queryFn: () => ordersService.getAll(undefined, undefined, undefined, undefined, undefined, undefined, undefined, 1, 8, 'VENTE'),
        refetchInterval: 60_000,
    });
    ...
    const barOptions = useMemo<ApexOptions>(() => ({
        ...
        xaxis: {
            categories: dailyStats.map(s =>
                new Date(s.date).toLocaleDateString(language === 'ar' ? 'ar-MA' : 'fr-FR', { weekday: 'short' })
            ),
        ...
        tooltip: {
            ...
            theme: 'light',
            style: { fontSize: '12px' },
            fixed: isMobile ? { enabled: true, position: 'topLeft', offsetX: 0, offsetY: -10 } : { enabled: false },
        },
        legend: { show: false },
        responsive: [
            {
    ...
    const now = new Date();
    const greet = tx.greeting(now.getHours(), (admin?.fullName ?? admin?.name ?? '').split(' ')[0] || 'Admin');

    // Skeleton card
    const SkCard = () => (
        <div style={{ borderRadius: '1rem', background: '#f1f5f9', height: '6rem', animation: 'pulse 1.5s ease-in-out infinite' }} />
    );
        ...
        <AdminLayout>
            <div style={{ padding: '0 0 2rem', direction: isRtl ? 'rtl' : 'ltr' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem', gap: '1rem' }}>
                    <div>
                        ...
                        <div style={{ fontSize: '0.8rem', color: '#94a3b8', marginTop: '0.2rem' }}>
                            {now.toLocaleDateString(language === 'ar' ? 'ar-MA' : 'fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}
                        </div>
                    </div>
                    <button
                        onClick={() => refetch()}
                    ...
                    >
                        ...
                        {!isLoading && !isFetching && <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#16a34a', display: 'inline-block' }} />}
                    </button>
                </div>
                ...
                <div style={{ ...sectionCard, marginBottom: '1.25rem', overflow: 'hidden' }}>
                    ...
                    {isLoading ? (
                        <div style={{ height: '12rem', background: '#f1f5f9', borderRadius: '0.75rem', animation: 'pulse 1.5s ease-in-out infinite' }} />
                    ) : (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(18rem, 1fr))', gap: '1.25rem', marginBottom: '1.25rem' }}>
                    ...
                    <div style={sectionCard}>
                        ...
                        {isLoading ? (
                            [0, 1, 2, 3, 4].map(i => (
                                <div key={i} style={{ height: '2.5rem', background: '#f1f5f9', borderRadius: '0.5rem', marginBottom: '0.5rem', animation: 'pulse 1.5s ease-in-out infinite' }} />
                            ))
                        ) : topProducts.slice(0, 5).length === 0 ? (
                            <div style={{ color: '#94a3b8', textAlign: 'center', padding: '2rem 0', fontSize: '0.875rem' }}>—</div>
                        ) : (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                {topProducts.slice(0, 5).map((p, idx) => (
                                    <div key={p.productId} style={{
                                        ...
                                        <span style={{
                                            fontSize: '1rem', fontWeight: 800, color: idx === 0 ? '#b45309' : '#94a3b8',
                                            minWidth: '1.5rem', textAlign: 'center',
                                        }}>
                                            {idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : `${idx + 1}.`}
                                        </span>
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <div style={{ fontWeight: 700, fontSize: '0.875rem', color: '#0f172a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.productName}</div>
                                        </div>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', flexShrink: 0 }}>
                                            <Package size={12} color="#16a34a" />
                                            <span style={{ fontSize: '0.875rem', fontWeight: 700, color: '#16a34a' }}>{p.sales} {tx.qty}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <div style={sectionCard}>
                    <div style={{ ...sectionTitle, justifyContent: 'space-between' }}>
                        ...
                        <button
                            ...
                            style={{
                                ...
                                color: '#2563eb', fontSize: '0.8125rem', fontWeight: 600,
                    ...
                    {isMobile ? (
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                            {isLoading ? (
                                [0, 1, 2, 3].map(i => (
                                    <div key={i} style={{ height: '4.5rem', background: '#f1f5f9', borderRadius: '0.75rem', animation: 'pulse 1.5s ease-in-out infinite' }} />
                                ))
                                    ...
                                    return (
                                        ...
                                        >
                                            <div style={{ minWidth: 0 }}>
                                                <div style={{ fontWeight: 700, fontSize: '0.875rem', color: '#0f172a' }}>#{order.orderNumber}</div>
                                                <div style={{ fontSize: '0.75rem', color: '#64748b', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '10rem' }}>
                                                    {order.customerName ?? '—'}
                                                </div>
                                            </div>
                                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: isRtl ? 'flex-start' : 'flex-end', gap: '0.25rem', flexShrink: 0 }}>
                                ...
                                <thead>
                                    <tr style={{ borderBottom: '2px solid #f1f5f9' }}>
                                        {[tx.orderNum, tx.client, tx.amount, tx.status].map(h => (
                                            <th key={h} style={{
                                                padding: '0.5rem 0.75rem', textAlign: isRtl ? 'right' : 'left',
                                    ...
                                    {isLoading ? (
                                            ...
                                            <tr key={i}>
                                                {[0, 1, 2, 3].map(j => (
                                                    <td key={j} style={{ padding: '0.75rem' }}>
                                                        <div style={{ height: '1rem', background: '#f1f5f9', borderRadius: '0.25rem', animation: 'pulse 1.5s ease-in-out infinite' }} />
                                                    </td>
                                                ))}
                                            </tr>
                                        ))
                                    ) : recentOrders.length === 0 ? (
                                        <tr><td colSpan={4} style={{ textAlign: 'center', padding: '2rem', color: '#94a3b8' }}>—</td></tr>
                                    ) : (
                                        recentOrders.map((order, idx) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <td style={{ padding: '0.75rem', fontWeight: 700, color: '#0f172a', whiteSpace: 'nowrap' }}>#{order.orderNumber}</td>
                                                    <td style={{ padding: '0.75rem', color: '#334155', maxWidth: '10rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                        {order.customerName ?? '—'}
                                                    </td>
                                                    <td style={{ padding: '0.75rem', fontWeight: 700, color: '#0f172a', whiteSpace: 'nowrap' }}>
                                                        {formatCurrency(order.total, language)}
                                                    </td>
