...
export default function RolesPage() {
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => { toastSuccess(t('roleCreated' as any)); queryClient.invalidateQueries({ queryKey: ['roles'] }); closeModal(); },
        onError: (e: any) => toastError(e.message),
    });

    const updateMutation = useMutation({
        mutationFn: ({ id, payload }: { id: number; payload: UpdateRolePayload }) => rolesService.update(id, payload),
        onSuccess: () => { toastSuccess(t('roleUpdated' as any)); queryClient.invalidateQueries({ queryKey: ['roles'] }); closeModal(); },
        onError: (e: any) => toastError(e.message),
    });

    const deleteMutation = useMutation({
        mutationFn: (id: number) => rolesService.remove(id),
        onSuccess: () => { toastSuccess(t('roleDeleted' as any)); queryClient.invalidateQueries({ queryKey: ['roles'] }); },
        onError: (e: any) => toastError(e.message),
    });
    ...
    const handleDelete = (role: Role) => {
        if (role.isSuperAdmin) { toastError(t('cannotDeleteSuperAdmin' as any)); return; }
    ...
    const permCountTemplate = (role: Role) => (
        <span style={{ fontSize: '0.8125rem', color: role.isSuperAdmin ? '#f59e0b' : '#235ae4', fontWeight: 600 }}>
            {role.isSuperAdmin ? t('allPermissions' as any) : `${role.permissions.length} / ${allPerms.length}`}
        </span>
    );
                ...
                <Modal
                    ...
                    footer={
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
                            <Button label={t('cancel' as any)} onClick={closeModal} text style={{ color: '#64748b' }} />
                            <Button
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', padding: '0.5rem 0' }}>
                        ...
                        <div>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                                ...
                                <div style={{ display: 'flex', gap: '0.375rem' }}>
                                    <Button label={t('selectAll' as any)} text size="small" onClick={selectAll} style={{ color: '#235ae4', fontSize: '0.75rem' }} />
                                    <Button label={t('deselectAll' as any)} text size="small" onClick={deselectAll} style={{ color: '#94a3b8', fontSize: '0.75rem' }} />
                                </div>
                            </div>
