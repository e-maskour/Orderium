...
const COLUMNS = [
  ...
  { field: 'inflow', header: 'Encaissements (MAD)', body: (row: Record<string, unknown>) => MAD(row, 'inflow') },
  { field: 'outflow', header: 'Décaissements (MAD)', body: (row: Record<string, unknown>) => MAD(row, 'outflow') },
...
const CashflowPage: React.FC = () => {
  ...
  const { data, isLoading, error, refetch } = useReport<ReportData>(() => analyticsService.getCashflow(filter));
  ...
  return (
    <ReportLayout
      ...
      exportButtons={<ExportButtons xlsxUrl={analyticsService.xlsxUrl(API_ROUTES.REPORTS.PAYMENTS.CASHFLOW + '/xlsx', filter)} xlsxFilename="tresorerie.xlsx" />}
    />
  );
};
