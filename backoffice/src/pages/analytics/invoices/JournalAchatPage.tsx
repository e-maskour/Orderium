...
const JournalAchatPage: React.FC = () => {
  ...
  const { data, isLoading, error, refetch } = useReport<ReportData>(() => analyticsService.getJournalAchat(filter));
  ...
  return (
    <ReportLayout
      ...
      subtitle="Toutes les factures fournisseurs"
      isLoading={isLoading}
      error={error}
      ...
      exportButtons={<ExportButtons xlsxUrl={analyticsService.xlsxUrl(API_ROUTES.REPORTS.INVOICES.JOURNAL_ACHAT + '/xlsx', filter)} xlsxFilename="journal-achat.xlsx" />}
    />
  );
};
