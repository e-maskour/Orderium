...
const COLUMNS = [
  ...
  { field: 'totalPurchases', header: 'Montant (MAD)', body: (row: Record<string, unknown>) => Number(row.totalPurchases).toLocaleString('fr-MA', { minimumFractionDigits: 2 }) },
];

const PurchasesTopSuppliersPage: React.FC = () => {
  const [filter, setFilter] = useState<ReportFilter>({ preset: 'this_month' });
  const { data, isLoading, error, refetch } = useReport<ReportData>(() => analyticsService.getPurchasesTopSuppliers(filter));
  ...
  return (
    <ReportLayout
      ...
      exportButtons={<ExportButtons xlsxUrl={analyticsService.xlsxUrl(API_ROUTES.REPORTS.PURCHASES.TOP_SUPPLIERS + '/xlsx', filter)} xlsxFilename="top-fournisseurs-achats.xlsx" />}
    />
  );
};
