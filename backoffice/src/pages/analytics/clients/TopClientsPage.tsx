...
const COLUMNS = [
  ...
  { field: 'totalRevenue', header: 'CA total (MAD)', body: (row: Record<string, unknown>) => MAD(row, 'totalRevenue') },
...
const TopClientsPage: React.FC = () => {
  ...
  const { data, isLoading, error, refetch } = useReport<ReportData>(() => analyticsService.getTopClients(filter));
  ...
  return (
    <ReportLayout
      ...
      exportButtons={<ExportButtons xlsxUrl={analyticsService.xlsxUrl(API_ROUTES.REPORTS.CLIENTS.TOP + '/xlsx', filter)} xlsxFilename="top-clients.xlsx" />}
    />
  );
};
