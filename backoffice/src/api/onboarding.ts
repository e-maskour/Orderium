...
export async function submitAdminAccount(data: CreateAdminPayload): Promise<AdminCreatedResponse> {
  const res = await apiClient.post<AdminCreatedResponse>(API_ROUTES.ONBOARDING.ADMIN, data, {
    skipAuth: true,
  } as any);
