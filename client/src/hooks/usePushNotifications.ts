...
// App type for client portal
const APP_TYPE = 'client';

// Local storage key for token
const TOKEN_STORAGE_KEY = 'orderium_fcm_token';
const PROJECT_STORAGE_KEY = 'orderium_fcm_project';
const PROJECT_ID = import.meta.env.VITE_FIREBASE_PROJECT_ID ?? '';

/**
 * Hook for managing push notifications in the Client Portal
 */
export function usePushNotifications(
  ...
  useEffect(() => {
    ...
    unsubscribeRef.current = onForegroundMessage((payload) => {
      ...
      // Show notification toast or handle in-app
      if (onNotification) {
  ...
  const registerTokenToServer = async (userId: number, token: string): Promise<boolean> => {
    try {
      ...
      await http(API_ROUTES.NOTIFICATIONS.DEVICE_TOKEN(userId), {
        method: 'POST',
        body: JSON.stringify({
          token,
          platform,
  ...
  const unregisterToken = useCallback(async (): Promise<boolean> => {
    ...
    try {
      await http(API_ROUTES.NOTIFICATIONS.DEVICE_TOKEN_DELETE, {
        method: 'DELETE',
  ...
  const refreshToken = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      if (token) {
        ...
        // Refresh on server
        await http(API_ROUTES.NOTIFICATIONS.DEVICE_TOKEN_REFRESH(token), {
          method: 'PATCH',
        });
      }
      return token;
