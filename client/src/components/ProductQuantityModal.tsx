...
export const ProductQuantityModal = ({ product, isOpen, onClose, initialQuantity = 0 }: ProductQuantityModalProps) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-lg max-w-[95vw] p-0 overflow-hidden" dir={dir}>
        {/* Header */}
        <div className="relative bg-gradient-to-br from-primary/10 to-primary/5 p-3 sm:p-4 border-b border-gray-200">
          <button
            onClick={handleClose}
            className="absolute top-3 sm:top-4 end-3 sm:end-4 w-8 h-8 rounded-full bg-white hover:bg-gray-100 flex items-center justify-center shadow-sm transition-all hover:scale-110"
          >
            <X className="w-4 h-4 text-gray-700" />
          </button>

          <div className="space-y-1.5 sm:space-y-2 pe-10 sm:pe-12">
          ...
          <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
            {numbers.map((num) => (
              <button
                ...
                className={cn(
                  "h-11 sm:h-12 text-base sm:text-lg font-semibold rounded-lg transition-all active:scale-95",
                  num === 'C'
