...
const PLAN_COLORS: Record<string, string> = {
    basic: 'border-indigo-400 bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300',
    pro: 'border-violet-400 bg-violet-50 text-violet-700 dark:bg-violet-900/30 dark:text-violet-300',
    enterprise: 'border-purple-400 bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300',
}
...
export function TenantForm({ onSubmit, loading = false, initial, editMode = false }: Props) {
    ...
    const validate = (): boolean => {
        ...
        if (form.contactEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.contactEmail?.toString() ?? '')) {
                    ...
                    {errors.slug
                        ...
                        : !editMode && (
                            <p className="mt-1 text-xs text-slate-400">
                                e.g. <code className="font-mono">{(form.slug as string) || '…'}-admin.domain.com</code>
                            </p>
                        )
                    }
                </div>
                        ...
                        {PLANS.map((p) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full rounded-lg border-2 px-3 py-1.5 text-xs font-semibold capitalize transition-all ${isSelected
                                            ? PLAN_COLORS[p]
