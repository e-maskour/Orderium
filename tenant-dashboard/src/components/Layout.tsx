...
export function Layout({ onLogout }: Props) {
  ...
  const expiringCount =
    data?.data?.filter((t) => t.status === 'trial' && (t.trialDaysRemaining ?? 99) <= 7).length ??
    0;
  ...
  const inactiveClass =
    'nav-link text-slate-600 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800/80 dark:hover:text-slate-100';
  ...
  const SidebarInner = () => (
    <>
      ...
      <nav className="flex-1 overflow-y-auto p-3 space-y-0.5">
        ...
        <NavLink to="/tenants" className={() => (isTenantsActive ? activeClass : inactiveClass)}>
        ...
        {expiringCount > 0 && (
          ...
          >
            <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-amber-500 text-[8px] font-extrabold text-white leading-none">
              !
            </span>
