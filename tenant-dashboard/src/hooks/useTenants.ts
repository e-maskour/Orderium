...
import type { ListTenantsParams, CreateTenantInput, UpdateTenantInput, CreatePaymentInput } from '../types/tenant'
...
export function useArchiveTenant() {
    const qc = useQueryClient()
    return useMutation({
        mutationFn: ({ id, confirmation, reason }: { id: number; confirmation: string; reason?: string }) =>
...
export function useValidatePayment() {
    const qc = useQueryClient()
    return useMutation({
        mutationFn: ({ paymentId, validatedBy }: { paymentId: string; tenantId: number; validatedBy?: string }) =>
...
export function useResetTenant() {
    ...
    return useMutation({
        ...
        onSuccess: (_data, { id }) =>
            qc.invalidateQueries({ queryKey: tenantKeys.stats(id) }),
    })
}
