...
describe('WarehouseService', () => {
  ...
  describe('create', () => {
    ...
    it('throws BadRequestException when code already exists', async () => {
      ...
      await expect(
        service.create({ name: 'Dup', code: 'WH-MAIN' }),
      ).rejects.toThrow(BadRequestException);
      await expect(
        service.create({ name: 'Dup', code: 'WH-MAIN' }),
      ).rejects.toThrow("Warehouse with code 'WH-MAIN' already exists");
    });
  });
  ...
  describe('findAll', () => {
    it('returns only active warehouses', async () => {
      const warehouses = [
        makeWarehouse(),
        makeWarehouse({ id: 2, code: 'WH-SEC' }),
      ];
  ...
  describe('update', () => {
    ...
    it('throws BadRequestException when new code already belongs to another warehouse', async () => {
      ...
      await expect(service.update(1, { code: 'WH-SEC' })).rejects.toThrow(
        BadRequestException,
      );
    });
