...
const makeWarehouse = (overrides: Partial<Warehouse> = {}): Warehouse =>
    ({ id: 1, name: 'Main Warehouse', code: 'WH-001', isActive: true, ...overrides } as Warehouse);

const makeProduct = (overrides: any = {}): Product =>
    ({ id: 1, name: 'Widget', code: 'WGT-001', isEnabled: true, ...overrides } as Product);

const makeAdjustment = (overrides: Partial<InventoryAdjustment> = {}): InventoryAdjustment =>
({
    id: 1,
...
const makeAdjustmentLine = (overrides: Partial<AdjustmentLine> = {}): AdjustmentLine =>
({
    id: 1,
    adjustmentId: 1,
    productId: 1,
...
describe('InventoryAdjustmentService', () => {
    ...
    beforeEach(async () => {
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                { provide: getRepositoryToken(InventoryAdjustment), useFactory: mockRepository },
                { provide: getRepositoryToken(AdjustmentLine), useFactory: mockRepository },
                ...
                { provide: getRepositoryToken(StockMovement), useFactory: mockRepository },
    ...
    describe('create', () => {
        ...
        it('creates adjustment with lines and calculates difference', async () => {
            ...
            jest.spyOn(service, 'findOne').mockResolvedValue(
                makeAdjustment({ lines: [line] }),
            );

            const result = await service.create({
                name: 'Count',
                warehouseId: 1,
                lines: [{ productId: 1, theoreticalQuantity: 100, countedQuantity: 95 }],
            });
        ...
        it('throws NotFoundException when a line product does not exist', async () => {
            ...
            await expect(
                service.create({
                    name: 'Count',
                    warehouseId: 1,
                    lines: [{ productId: 999, theoreticalQuantity: 10, countedQuantity: 10 }],
                }),
            ).rejects.toThrow(NotFoundException);
            await expect(
                service.create({
        ...
        it('calculates zero difference correctly', async () => {
            ...
            await service.create({
                name: 'Count',
                warehouseId: 1,
                lines: [{ productId: 1, theoreticalQuantity: 100, countedQuantity: 100 }],
            });
    ...
    describe('findOne', () => {
        ...
        it('throws NotFoundException for non-existing adjustment', async () => {
            ...
            await expect(service.findOne(999)).rejects.toThrow('Inventory adjustment with ID 999 not found');
        });
    });
    ...
    describe('update', () => {
        it('updates name and notes on a DRAFT adjustment', async () => {
            ...
            jest.spyOn(service, 'findOne').mockResolvedValueOnce(adjustment).mockResolvedValueOnce(
                makeAdjustment({ name: 'Updated Name', notes: 'new notes' }),
            );

            const result = await service.update(1, { name: 'Updated Name', notes: 'new notes' });
        ...
        it('throws BadRequestException when updating a DONE adjustment', async () => {
            ...
            await expect(service.update(1, { name: 'X' })).rejects.toThrow('Cannot update validated adjustment');
        });
    });
    ...
    describe('adjustment logic edge cases', () => {
        it('handles lines with fractional quantities correctly', async () => {
            ...
            await service.create({
                ...
                warehouseId: 1,
                lines: [{ productId: 1, theoreticalQuantity: 10.5, countedQuantity: 10.25 }],
            });
        ...
        it('handles large counted quantity (surplus)', async () => {
            ...
            await service.create({
                name: 'Surplus Count',
                warehouseId: 1,
                lines: [{ productId: 1, theoreticalQuantity: 0, countedQuantity: 9999 }],
            });
