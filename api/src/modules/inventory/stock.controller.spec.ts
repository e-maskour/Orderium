...
describe('StockController', () => {
    ...
    describe('getProductStock', () => {
        ...
        it('propagates NotFoundException from service', async () => {
            mockStockService.getProductStock.mockRejectedValue(new NotFoundException('Product not found'));

            await expect(controller.getProductStock('999')).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('getWarehouseStock', () => {
        ...
        it('propagates NotFoundException from service', async () => {
            mockStockService.getWarehouseStock.mockRejectedValue(new NotFoundException('Warehouse not found'));

            await expect(controller.getWarehouseStock('999')).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('getStockValue', () => {
        it('returns stock value and product count', async () => {
            mockStockService.getStockValue.mockResolvedValue({ totalValue: 50000, productCount: 120 });
    ...
    describe('reserveStock', () => {
        it('reserves stock successfully', async () => {
            const reservedQuant = { ...stockQuantStub, reservedQuantity: 40, availableQuantity: 60 };
        ...
        it('propagates BadRequestException on insufficient stock', async () => {
            ...
            await expect(
                controller.reserveStock({ productId: 1, warehouseId: 1, quantity: 9999 }),
    ...
    describe('unreserveStock', () => {
        it('releases reserved stock successfully', async () => {
            const unReservedQuant = { ...stockQuantStub, reservedQuantity: 0, availableQuantity: 100 };
