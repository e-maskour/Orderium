  ...
  async generateNext(
    ...
    try {
      ...
      sequence = {
        ...
        prefix: String((raw['prefix'] as string) ?? ''),
        suffix: String((raw['suffix'] as string) ?? ''),
        ...
        formatTemplate: String((raw['format_template'] as string) ?? ''),
        resetPeriod: String((raw['reset_period'] as string) ?? 'yearly'),
        currentPeriodKey: String((raw['current_period_key'] as string) ?? ''),
