...
import { Tenant, TenantStatus, SubscriptionPlan } from '../tenant/tenant.entity';
...
export class TenantLifecycleService {
    ...
    async deletePermanently(
        ...
        await this.applyStatusChange(tenant, 'deleted', 'Permanent deletion', performedBy);
    ...
    async createPayment(
        ...
        await this.logActivity(
            tenantId,
            'payment_recorded',
            { amount: dto.amount, currency: dto.currency ?? 'MAD', plan: dto.planName },
            performedBy,
        );
        if (dto.validateImmediately) {
            return this.validatePayment(saved.id, { validatedBy: performedBy }, performedBy);
        }
        return saved;
    }
    ...
    async expireTrials(): Promise<void> {
        ...
        for (const tenant of expired) {
            await this.applyStatusChange(tenant, 'expired', 'Trial period ended automatically');
    ...
    async expireSubscriptions(): Promise<void> {
        ...
        for (const tenant of expired) {
            await this.applyStatusChange(tenant, 'expired', 'Subscription ended automatically');
    ...
    private async destroyTenantInfra(tenant: Tenant): Promise<void> {
        ...
        try {
            ...
            this.logger.error(`Failed to drop DB for '${tenant.slug}': ${(err as Error).message}`);
        }
        ...
        try {
            ...
            this.logger.error(`Failed to delete bucket for '${tenant.slug}': ${(err as Error).message}`);
        }
    ...
    private async setRedisStatus(slug: string, status: string): Promise<void> {
        try {
            ...
            this.logger.warn(`Failed to set Redis status for '${slug}': ${(err as Error).message}`);
        }
    }
