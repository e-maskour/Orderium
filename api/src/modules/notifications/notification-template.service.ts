...
export class NotificationTemplateService {
    ...
    async update(
        key: string,
        data: Partial<Pick<NotificationTemplate, 'titleFr' | 'bodyFr' | 'titleAr' | 'bodyAr' | 'enabled' | 'priority'>>,
    ...
    async toggleEnabled(key: string, enabled: boolean): Promise<NotificationTemplate | null> {
        ...
        try {
            ...
            for (const recipient of recipients) {
                await this.dispatchToRecipient(
                    recipient,
                    { key, title, body, priority, variables, metadata },
                );
            }
        } catch (error) {
            this.logger.error(
    ...
    private async dispatchToRecipient(
        ...
        const pushPayload = { title, body, data: { type: key, ...this.stringifyValues(metadata) } };

        switch (recipient.type) {
            case 'admins': {
                await this.pushNotificationService.sendToAdmins(pushPayload).catch(this.logPushError.bind(this));
            ...
            case 'customer': {
                ...
                const customerUserId = await this.pushNotificationService.getUserIdByCustomerId(
            ...
            case 'delivery': {
                ...
                await this.pushNotificationService
                    ...
                    .catch(this.logPushError.bind(this));
                const deliveryUserId = await this.pushNotificationService.getUserIdByDeliveryId(
    ...
    async seedDefaultsIfNeeded(): Promise<void> {
        ...
        if (count === 0) {
            ...
            this.logger.log(`Seeded ${DEFAULT_NOTIFICATION_TEMPLATES.length} default notification templates`);
        }
    }

    private async seedTemplate(def: NotificationTemplateDefault): Promise<NotificationTemplate> {
    ...
    private stringifyValues(obj: Record<string, unknown>): Record<string, string> {
    ...
    private logPushError(err: unknown): void {
        this.logger.warn(`Push notification failed (non-fatal): ${(err as Error)?.message}`);
    }
}
