...
export class InvoiceReportsController {
  ...
  @Get('journal-vente')
  @ApiOperation({ summary: 'Sales invoice journal (journal des factures vente)' })
  @ApiResponse({ status: 200 })
  async getJournalVente(@Query() filter: InvoiceReportFilterDto) {
    return ApiRes(RPT.INVOICE_JOURNAL_VENTE, await this.service.getJournalVente(filter));
  }

  @Get('journal-achat')
  @ApiOperation({ summary: 'Purchase invoice journal (journal des factures achat)' })
  @ApiResponse({ status: 200 })
  async getJournalAchat(@Query() filter: InvoiceReportFilterDto) {
    return ApiRes(RPT.INVOICE_JOURNAL_ACHAT, await this.service.getJournalAchat(filter));
  }
  ...
  async getTvaSummary(@Query() filter: InvoiceReportFilterDto) {
    return ApiRes(RPT.INVOICE_TVA_SUMMARY, await this.service.getTvaSummary(filter));
  }
  ...
  async getOutstanding(@Query() filter: ReportFilterDto) {
    return ApiRes(RPT.INVOICE_OUTSTANDING, await this.service.getOutstanding(filter));
  }

  @Get('aging-balance')
  @ApiOperation({ summary: 'Aging balance by partner (0 / 1-30 / 31-60 / 61-90 / +90 days)' })
  @ApiResponse({ status: 200 })
  async getAgingBalance(@Query() filter: AgingReportFilterDto) {
    return ApiRes(RPT.INVOICE_AGING, await this.service.getAgingBalance(filter));
  }
  ...
  async getJournalVenteXlsx(@Query() filter: InvoiceReportFilterDto, @Res() res: Response) {
    const buffer = await this.service.getJournalVenteXlsx(filter);
    res.setHeader('Content-Disposition', 'attachment; filename="journal-ventes.xlsx"');
    res.end(buffer);
  }
  ...
  async getTvaSummaryXlsx(@Query() filter: InvoiceReportFilterDto, @Res() res: Response) {
    const buffer = await this.service.getTvaSummaryXlsx(filter);
    res.setHeader('Content-Disposition', 'attachment; filename="recap-tva.xlsx"');
    res.end(buffer);
  }
}
