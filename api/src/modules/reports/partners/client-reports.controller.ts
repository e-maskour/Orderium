...
export class ClientReportsController {
  ...
  async getCustomerAging(@Query() filter: AgingReportFilterDto) {
    return ApiRes(
      RPT.CLIENTS_AGING,
      await this.service.getCustomerAging(filter),
    );
  }
  ...
  async getInactiveCustomers(@Query() filter: ReportFilterDto) {
    return ApiRes(
      ...
      await this.service.getInactiveCustomers(filter),
    );
  }
  ...
  async getCustomerStatement(@Query() filter: PartnerStatementFilterDto) {
    return ApiRes(
      ...
      await this.service.getCustomerStatement(filter),
    );
  }
  ...
  async getTopCustomersXlsx(
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename="top-clients.xlsx"',
    );
    res.end(buffer);
  }
}
