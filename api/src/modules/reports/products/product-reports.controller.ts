...
export class ProductReportsController {
  ...
  async getProductPerformance(@Query() filter: ReportFilterDto) {
    return ApiRes(RPT.PRODUCTS_PERFORMANCE, await this.service.getProductPerformance(filter));
  }
  ...
  async getMarginAnalysis(@Query() filter: ReportFilterDto) {
    return ApiRes(RPT.PRODUCTS_MARGIN, await this.service.getMarginAnalysis(filter));
  }
  ...
  async getNeverSoldProducts(@Query() filter: ReportFilterDto) {
    return ApiRes(RPT.PRODUCTS_NEVER_SOLD, await this.service.getNeverSoldProducts(filter));
  }

  @Get('performance/xlsx')
  @Header('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
  @ApiOperation({ summary: 'Export product performance as XLSX' })
  async getProductPerformanceXlsx(@Query() filter: ReportFilterDto, @Res() res: Response) {
    const buffer = await this.service.getProductPerformanceXlsx(filter);
    res.setHeader('Content-Disposition', 'attachment; filename="performance-produits.xlsx"');
    res.end(buffer);
  }
}
