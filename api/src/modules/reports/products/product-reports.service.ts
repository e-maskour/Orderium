...
export class ProductReportsService {
  ...
  async getProductPerformance(filter: ReportFilterDto) {
    ...
    return this.withCache(key, async () => {
      ...
      const grandRevenue = allRaw.reduce(
        (s, r) => s + Number(r.totalRevenue),
        0,
      );
      const rows = allRaw
        ...
        .map((r) => ({
          ...
          revenueShare:
            grandRevenue > 0
              ? Math.round((Number(r.totalRevenue) / grandRevenue) * 100)
              : 0,
        }));
    ...
    return this.withCache(key, async () => {
      ...
      return {
        kpis: {
          ...
          totalMargin: allRaw.reduce(
            (s, r) => s + (Number(r.totalRevenue) - Number(r.totalCost)),
            0,
          ),
          avgMarginRate:
            total > 0
              ? Math.round(rows.reduce((s, r) => s + r.marginPct, 0) / total)
              : 0,
        },
        chart: {
          type: 'bar',
          ...
          series: [
            {
              ...
              data: allRaw
                .slice(0, 10)
                .map((r) => Number(r.totalRevenue) - Number(r.totalCost)),
            },
          ],
        },
        rows,
  ...
  async getNeverSoldProducts(filter: ReportFilterDto) {
    ...
    return this.withCache(key, async () => {
      ...
      const allProducts = await qb
        .select(['p.id', 'p.name', 'p.code', 'p.price', 'p.cost', 'p.stock'])
        .getMany();
      ...
      return {
        kpis: {
          ...
          immobilisedValue: neverSold.reduce(
  ...
  async getProductPerformanceXlsx(filter: ReportFilterDto): Promise<Buffer> {
    const data = await this.getProductPerformance({
      ...filter,
      page: 1,
      perPage: 10_000,
    });
