...
import { DatePreset, InvoiceReportFilterDto, ReportFilterDto, AgingReportFilterDto } from '../dto/report-filter.dto';
...
describe('InvoiceReportsService', () => {
  ...
  const defaultFilter: InvoiceReportFilterDto = { preset: DatePreset.THIS_MONTH, page: 1, perPage: 50 };
  ...
  describe('getJournalVente', () => {
    ...
    it('maps invoice KPIs correctly', async () => {
      const kpiRaw = { totalHT: '50000.00', totalTVA: '10000.00', totalPaid: '30000.00', totalRemaining: '20000.00', count: '15' };
      const invoiceRows = [
        { id: 1, documentNumber: 'FAC-001', date: '2025-03-10', customerName: 'Client A', subtotal: 40000, tax: 8000, total: 48000, paidAmount: 20000, remainingAmount: 28000, status: 'PARTIAL' },
      ];

      mockInvoiceRepo.createQueryBuilder
        .mockReturnValueOnce(makeQb({ getRawOne: jest.fn().mockResolvedValue(kpiRaw) }))
        .mockReturnValueOnce(makeQb({ getManyAndCount: jest.fn().mockResolvedValue([invoiceRows, 1]) }));
  ...
  describe('getTvaSummary', () => {
    it('returns all 5 Moroccan TVA rate rows even when DB is empty', async () => {
      mockInvoiceItemRepo.createQueryBuilder.mockReturnValue(makeQb({
      ...
      expect(result.rows.every((r) => r.venteHT === 0 && r.achatHT === 0)).toBe(true);
      expect(result.kpis.tvaAVerser).toBe(0);
    });

    it('pivots TVA rows correctly', async () => {
      const rawItems = [
        { taxRate: '20', direction: 'VENTE', totalHT: '10000.00', totalTVA: '2000.00' },
        { taxRate: '20', direction: 'ACHAT', totalHT: '5000.00', totalTVA: '1000.00' },
      ];
      mockInvoiceItemRepo.createQueryBuilder.mockReturnValue(makeQb({
  ...
  describe('getOutstanding', () => {
    ...
    it('counts overdue invoices', async () => {
      ...
      const invoiceRows = [
        { id: 1, documentNumber: 'FAC-002', date: '2025-01-01', dueDate: yesterday.toISOString(), direction: 'VENTE', customerName: 'Client B', supplierName: null, total: 5000, paidAmount: 0, remainingAmount: 5000, status: 'UNPAID' },
      ];

      mockInvoiceRepo.createQueryBuilder
        .mockReturnValueOnce(makeQb({ getManyAndCount: jest.fn().mockResolvedValue([invoiceRows, 1]) }))
        .mockReturnValueOnce(makeQb({ getRawOne: jest.fn().mockResolvedValue({ totalRemaining: '5000', count: '1' }) }));
