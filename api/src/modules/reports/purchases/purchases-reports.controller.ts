...
export class PurchasesReportsController {
  ...
  async getByPeriod(@Query() filter: SalesReportFilterDto) {
    return ApiRes(RPT.PURCHASES_BY_PERIOD, await this.service.getPurchasesByPeriod(filter));
  }
  ...
  async getTopSuppliers(@Query() filter: SalesReportFilterDto) {
    return ApiRes(RPT.PURCHASES_TOP_SUPPLIERS, await this.service.getTopSuppliers(filter));
  }
  ...
  async getByProduct(@Query() filter: SalesReportFilterDto) {
    return ApiRes(RPT.PURCHASES_BY_PRODUCT, await this.service.getPurchasesByProduct(filter));
  }

  @Get('by-period/xlsx')
  @Header('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
  @ApiOperation({ summary: 'Export purchases report as XLSX' })
  async getPurchasesXlsx(@Query() filter: SalesReportFilterDto, @Res() res: Response) {
    const buffer = await this.service.getPurchasesXlsx(filter);
    res.setHeader('Content-Disposition', 'attachment; filename="rapport-achats.xlsx"');
    res.end(buffer);
  }
}
