...
describe('ProductsService', () => {
    ...
    beforeEach(async () => {
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                { provide: getRepositoryToken(UnitOfMeasure), useFactory: mockRepository },
    ...
    describe('create', () => {
        it('creates a product without categories', async () => {
            ...
            expect(productRepo.create).toHaveBeenCalledWith(expect.objectContaining({ name: 'Widget' }));
            expect(result.name).toBe('Widget');
        });

        it('creates a product and attaches categories', async () => {
            const dto = { name: 'Widget', price: 10, categoryIds: [1, 2] };
            const categories = [makeCategory({ id: 1 }), makeCategory({ id: 2, name: 'Tools' })];
    ...
    describe('findOne', () => {
        ...
        it('caches product after first database fetch', async () => {
            ...
            expect(mockCacheManager.set).toHaveBeenCalledWith('product:1', product, 300_000);
        });
    });
    ...
    describe('findAll', () => {
        ...
        it('applies negative stock filter', async () => {
            ...
            expect(productRepo._qb.andWhere).toHaveBeenCalledWith('product.stock < 0');
        });
        ...
        it('applies isService filter', async () => {
            await service.findAll(1, 10, undefined, undefined, undefined, undefined, true);
    ...
    describe('update', () => {
        ...
        it('updates categories when categoryIds are provided', async () => {
            ...
            productRepo.save.mockResolvedValue({ ...existing, categories: newCategories });

            jest.spyOn(service, 'findOne').mockResolvedValue(makeProduct({ categories: newCategories }));
        ...
        it('clears all categories when empty categoryIds array is provided', async () => {
            ...
            jest.spyOn(service, 'findOne').mockResolvedValue(makeProduct({ categories: [] }));
    ...
    describe('remove', () => {
        ...
        it('throws ConflictException when product is used in invoices', async () => {
            ...
            await expect(service.remove(1)).rejects.toThrow('Cannot delete product that is used in invoices');
        });
