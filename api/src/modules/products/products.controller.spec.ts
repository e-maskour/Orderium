...
jest.mock('sharp', () => jest.fn(() => ({ resize: jest.fn().mockReturnThis(), toBuffer: jest.fn() })));
...
describe('ProductsController', () => {
    ...
    describe('filterProducts', () => {
        it('returns paginated products with default page and perPage', async () => {
            ...
            expect(mockProductsService.findAll).toHaveBeenCalledWith(
                1, 50, undefined, undefined, undefined, undefined, undefined,
            );
        ...
        it('applies search filter from body', async () => {
            mockProductsService.findAll.mockResolvedValue({ products: [], count: 0, totalCount: 0 });

            await controller.filterProducts({ search: 'widget' } as any);

            expect(mockProductsService.findAll).toHaveBeenCalledWith(
                1, 50, 'widget', undefined, undefined, undefined, undefined,
            );
        });
        ...
        it('clamps perPage to maximum 100', async () => {
            ...
            expect(mockProductsService.findAll).toHaveBeenCalledWith(
                1, 100, undefined, undefined, undefined, undefined, undefined,
            );
        });
    ...
    describe('update', () => {
        it('updates product and returns updated data', async () => {
            ...
            const result = await controller.update(1, { name: 'Updated Widget' } as any);

            expect(mockProductsService.update).toHaveBeenCalledWith(1, { name: 'Updated Widget' });
