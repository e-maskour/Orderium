...
export class MigrationsController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Migration status retrieved for all tenants',
  })
  async getAllStatus() {
  ...
  @Get('logs')
  @ApiOperation({
    summary: 'Get migration execution logs (last 50 by default)',
  })
  ...
  async getTenantStatus(@Param('tenantId', ParseIntPipe) tenantId: number) {
    const data =
      await this.migrationsService.getTenantMigrationStatusById(tenantId);
  ...
  async runForTenant(@Param('tenantId', ParseIntPipe) tenantId: number) {
    this.logger.log(
      `Super admin triggered migration run for tenant #${tenantId}`,
    );
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Revert the last applied migration for a specific tenant',
  })
  ...
  async revertForTenant(@Param('tenantId', ParseIntPipe) tenantId: number) {
    this.logger.log(
      `Super admin triggered migration revert for tenant #${tenantId}`,
    );
    const data =
      await this.migrationsService.revertLastMigrationForTenant(tenantId);
