 ...
 * migration applied.
 */
export class AddShareTokenColumnsIfNotExists1774619375000 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "invoices" DROP COLUMN IF EXISTS "shareTokenExpiry"`,
    );
    await queryRunner.query(
      `ALTER TABLE "invoices" DROP CONSTRAINT IF EXISTS "UQ_invoices_shareToken"`,
    );
    await queryRunner.query(
      `ALTER TABLE "invoices" DROP COLUMN IF EXISTS "shareToken"`,
    );

    await queryRunner.query(
      `ALTER TABLE "orders" DROP COLUMN IF EXISTS "shareTokenExpiry"`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" DROP CONSTRAINT IF EXISTS "UQ_orders_shareToken"`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" DROP COLUMN IF EXISTS "shareToken"`,
    );
  }
}
