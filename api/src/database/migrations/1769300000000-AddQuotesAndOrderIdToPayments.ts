...
export class AddQuotesAndOrderIdToPayments1769300000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.createIndex('quote_items', new TableIndex({
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        if (paymentsTable) {
            const orderFk = paymentsTable.foreignKeys.find(fk => fk.columnNames.indexOf('orderId') !== -1);
            if (orderFk) {
        ...
        if (quoteItemsTable) {
            const quoteFk = quoteItemsTable.foreignKeys.find(fk => fk.columnNames.indexOf('quoteId') !== -1);
            if (quoteFk) {
                await queryRunner.dropForeignKey('quote_items', quoteFk);
            }

            const productFk = quoteItemsTable.foreignKeys.find(fk => fk.columnNames.indexOf('productId') !== -1);
            if (productFk) {
        ...
        if (quotesTable) {
            const customerFk = quotesTable.foreignKeys.find(fk => fk.columnNames.indexOf('customerId') !== -1);
            if (customerFk) {
