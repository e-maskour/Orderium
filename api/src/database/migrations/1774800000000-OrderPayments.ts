...
export class OrderPayments1774800000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        if (paymentsTable) {
            const orderIdFk = paymentsTable.foreignKeys.find(
                (fk) => fk.columnNames.includes('orderId') || fk.columnNames.includes('order_id'),
            );
            if (orderIdFk) {
            ...
            if (hasOrderId) {
                await queryRunner.dropColumn(
                    'payments',
                    paymentsTable.columns.find((c) => c.name === 'orderId' || c.name === 'order_id')!.name,
                );
            }
        }
        ...
        if (ordersTable) {
            const hasPaidAmount = ordersTable.columns.some((c) => c.name === 'paidAmount');
            if (!hasPaidAmount) {
            ...
            const hasRemainingAmount = ordersTable.columns.some((c) => c.name === 'remainingAmount');
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        if (ordersTable) {
            ...
            const hasRemainingAmount = ordersTable.columns.some((c) => c.name === 'remainingAmount');
            if (hasRemainingAmount) await queryRunner.dropColumn('orders', 'remainingAmount');
        }
    }
}
