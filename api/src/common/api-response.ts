...
export function ApiRes<T>(def: ResponseDef, data: T, metadata?: Record<string, any> | null): ApiResult<T> {
    return new ApiResult(data, def.message, def.code, def.status, metadata ?? null);
}

/** Wrap a paginated list with a specific ResponseDef */
export function ApiPaginated<T>(def: ResponseDef, data: T[], pagination: PaginationMeta): ApiResult<T[]> {
